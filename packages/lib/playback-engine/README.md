...
- **Owns**: Audio playback (HTML5 `<audio>` element management), playback state (playing, paused, stopped, current time, duration), seek operations, audio concatenation (for multi-chunk sessions or snips), playback events (play, pause, ended, timeupdate, error)
