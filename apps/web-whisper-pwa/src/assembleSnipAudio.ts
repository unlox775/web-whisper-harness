 ...
 * concatenating those refs (the previous PWA path) sent overlapping windows to
 * Groq and produced ~3–4s phrase repeats (BLT-like, but a full chunk long).
 */
...
export function sliceDuration(slice: AudioSlicePlan): number {
 ...
 */
export function chunksOverlappingSnip(
  snip: SnipTimeRange,
  chunks: ChunkTimeRange[]
): ChunkTimeRange[] {
 ...
 */
export function planSnipAudioSlices(
  snip: SnipTimeRange,
  chunks: ChunkTimeRange[]
): AudioSlicePlan[] {
  const overlapping = chunksOverlappingSnip(snip, chunks);
  const plans: AudioSlicePlan[] = [];
...
export function describeSnipTranscriptionJobs(
  snips: SnipTimeRange[],
  chunks: ChunkTimeRange[]
): SnipTranscriptionJob[] {
...
export function transcriptionWindowsOverlap(jobs: SnipTranscriptionJob[]): boolean {
  const windows = jobs
    .flatMap((job) =>
...
export function encodePcmWav(channelData: Float32Array, sampleRate: number): Blob {
...
export function wavDurationSeconds(blobBytes: ArrayBuffer): number {
...
async function defaultDecode(blob: Blob): Promise<DecodePcm> {
  const AudioCtx =
 ...
 */
export function exclusiveChunkIdsForSnip(
  snip: SnipTimeRange,
  chunks: ChunkTimeRange[]
): string[] {
  const overlapping = chunksOverlappingSnip(snip, chunks);
...
async function loadChunks(
  snip: SnipTimeRange,
  options: AssembleSnipAudioOptions
): Promise<ChunkTimeRange[]> {
...
function concatMp3(chunks: ChunkTimeRange[], ids: string[]): Blob {
  const wanted = new Set(ids);
  const blobs = chunks
    .filter((chunk) => wanted.has(chunk.id) && chunk.blob && chunk.blob.size > 0)
    .map((chunk) => chunk.blob as Blob);
 ...
 */
export async function assembleSnipTranscriptionBlob(
  snip: SnipTimeRange,
  options: AssembleSnipAudioOptions = {}
): Promise<AssembledSnipAudio> {
