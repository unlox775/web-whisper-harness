...
**Viewport:** iPhone 12 Pro (390×844 CSS px, 3× PNG)

## What was verified

1. **Developer mode off**
   - Home shows CAPTURE + empty session list. No LIBRARY card. No **Import session zip**. No ladybug.
   - Settings → App shows **Enable developer mode** unchecked. No Session archive block. No **Import session zip**.
   - Session Detail → Debug (`?screenshot=session-snips`) shows Chunks / Snips pills. No **Export session zip**. No debug-include checkbox.

2. **Developer mode on**
   - Home still has no import control (Settings-only). Ladybug console button appears.
...
- `documentation/qa/pwa-session-zip-dev-on-home.png` — Home still has no import when developer mode is on
